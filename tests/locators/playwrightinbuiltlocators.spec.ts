...
/*
    // 1. page.getByAltText() - identifies images or svg (and similar elements) based on the alt attribute.
...
//or
    ...
    await expect(page.getByText(/Welcome\s+To\s+Our\s+Store/i)).toBeVisible();   //regular expression(ignore case sensitive) */
...
// 4. page.getByLabel() - Locate form control by label's text
 ...
 //page.getByLabel('First name:').type("John");  // type is deprecated
  await page.getByLabel('First name:').fill("Roshan");  
...
// 5. page.getByPlaceholder() - Finds element with a given placeholder text.
  // Best for inputs without a label but having a placeholder.(Placeholder is the attributes)
...
await page.goto("http://127.0.0.1:5500/tests/locators/app.html");
 ...
 //--------------------------------------------------------------------------------------------------------------------------------
// 7. page.getByTestId() : Locate an element based on its data-testid attribute (other attributes can be configured)rarely used
