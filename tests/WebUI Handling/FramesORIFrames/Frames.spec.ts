...
test("frames demo",async ({page})=>{
  ...
  const frames=page.frames();//frames() returns array of frames
  console.log("Number of frames:", frames.length)//7

   //---- Approach 1: using page.frame() ----
/*   const frame=  page.frame({ url:"https://ui.vision/demo/webtest/frames/frame_1.html"});//key and value pair
...
//OR
//  const inputbox=page.frameLocator("[src='frame_1.html']").locator("[name='mytext1']").fill("Roshan")
