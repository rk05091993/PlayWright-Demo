...
test("Single Select Drop down",async ({page})=>{
 ...
 await page.goto('https://testautomationpractice.blogspot.com/')
  ...
  //1) select option from the drop down ( 4 ways)
   ...
   // await page.locator('#country').selectOption({index:3});   // by using index (manually count the index starting from 0)
...
//2) check number of options in the dropdown(count)
 ...
 //3) check an option present in the dropdown
    ...
    const optionsText:string[]=(await dropdownOptions.allTextContents()).map(text1=>text1.trim());   
