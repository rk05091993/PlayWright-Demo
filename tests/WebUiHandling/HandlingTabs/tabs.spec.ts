...
test("handle tabs",async()=>{
   ...
   // parentPage.locator("button:has-text('New Tab')").click(); // opens new tab/new page
    ...
    const [childPage]=await Promise.all([context.waitForEvent('page'),parentPage.locator("button:has-text('New Tab')").click()]);   //This is preffered


   //Appraoch 1: switch between pages and get titles ( using context)
   //----------------------------------------------------------------
   //Note : This approach is preferred when we have multiple pages 
   ...
   // console.log("Title of the Child page:", await pages[1].title());
    ...
    //-------------------------
    //Note: This approach is preffered when we only 2 pages because we know which page is parent page and which one is child page
